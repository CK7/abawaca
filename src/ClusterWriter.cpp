...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
void ClusterWriter::write(const ClusterData& clustering_db, const set<size_t>& scafs, const set<size_t>& raw_dps, const char* cluster_name) const
{
	char	scaf_stats_file[1024] = {};
	char	scaf_file[1024] = {};

	sprintf(scaf_stats_file, "%s/%s.scaf-stats.txt", out_dir, cluster_name);
	sprintf(scaf_file, "%s/%s.scaf-cluster.txt", out_dir, cluster_name);
...
void ClusterWriter::write_scaffolds(const set<size_t>& scafs, const char* out_file, const char* cluster_name) const
{
	...
	for(set<size_t>::const_iterator vit = scafs.begin(); vit != scafs.end(); vit++)
		fprintf(fp, "%s\t%s\n", scaf_db.scaf_id2name(*vit).c_str(), cluster_name);
	fclose(fp);
}

//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
void ClusterWriter::write_scaffold_stats(const ClusterData& clustering_db, const set<size_t>& scafs, const set<size_t>& raw_dps, const char* out_file) const
{
	...
	for(set<size_t>::const_iterator vit=raw_dps.begin(); vit != raw_dps.end(); vit++) {
...
void ClusterEsomWriter::write_clustering_info_file(const ClusterData& clustering_db, const char* cluster_name) const
{
	...
	sprintf(lrn_file, "%s/%s.lrn", out_dir, cluster_name);
	...
	const set<size_t>& dps = clustering_db.datapoints();
        for(set<size_t>::const_iterator vit=dps.begin(); vit!=dps.end(); vit++) {
                fprintf(fp, "%lu", *vit);
                for(size_t i=1; i<=clustering_db.ndimensions(); i++)
