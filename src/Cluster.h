...
public:
	...
	set<size_t>::const_iterator		dps_begin() const			{return dps.begin();}
	set<size_t>::const_iterator		dps_end() const				{return dps.end();}
	const set<size_t>&			get_dps() const				{return dps;}
	size_t					ndps() const				{return dps.size();}

	/* Manipulate assigned scafs, i.e. scaffolds that were assigned to the cluster by the user */
	const Cluster&				add_assigned_scaf(size_t scaf)		{assigned_scafs.insert(scaf);}
	const Cluster&				remove_assigned_scaf(size_t scaf)	{assigned_scafs.erase(scaf);}
